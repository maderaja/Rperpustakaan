...
$petugas = query("SELECT * FROM tb_petugas ORDER BY id_petugas DESC LIMIT $awalData, $jumlahDataPerHalaman");
            ...
            <div class="card-body">
              ...
              <?php for ($i = 1; $i <= $jumlahHalaman; $i++) : ?>
                <?php if ($i == $halamanAktif) : ?>
                  <a href="?halaman=<?= $i; ?>" style="font-weight: bold; color: white;" class="btn btn-primary"><?= $i; ?></a>
                <?php else : ?>
                  <a href="?halaman=<?= $i; ?>" class="btn btn-warning"><?= $i; ?></a>
                <?php endif; ?>
              <?php endfor; ?>
              ...
              <table class="table table-bordered table-striped text-center mt-3">
                ...
                <?php foreach ($petugas as $row) : ?>
                  <tr>
                    <td><?= $i ?></td>
                    <td>
                      <a href="../ubah/ubah_petugas.php?id=<?= $row["id_petugas"]; ?>" class="btn btn-warning"><i class="bi bi-pencil-square"></i> ubah</a>
                      <a href="../hapus/hapus_petugas.php?id=<?= $row["id_petugas"]; ?>" onclick="return confirm('anda yakin akan menghapus data ini?');" class="btn btn-danger"><i class="bi bi-trash-fill"></i> hapus</a>
                    </td>
                    <td><img src="../../img/petugas/<?= $row["foto"] ?>" width="70"></td>
                    <td><?= $row["nama"]; ?></td>
                    <td><?= $row["username"]; ?></td>
                    <td><?= $row["email"]; ?></td>
                  </tr>
