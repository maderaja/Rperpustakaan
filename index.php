...
if (isset($_POST["login"])) {
  ...
  $siswa = mysqli_query($conn, "SELECT * FROM tb_siswa WHERE username = '$username'");
  $petugas = mysqli_query($conn, "SELECT * FROM tb_petugas WHERE username = '$username'");
  $admin = mysqli_query($conn, "SELECT * FROM tb_admin WHERE username = '$username'");
  ...
  if (mysqli_num_rows($siswa) === 1) {
    ...
    if (password_verify($password, $rowSiswa["password"])) {
      ...
      if (isset($_POST["remember"])) {
        // buat cookie
        setcookie('id', $rowSiswa['id_siswa'], time() + 120);
        setcookie('key', $rowSiswa['username'], time() + 120);
        setcookie('nama', $rowSiswa['nama'], time() + 120);
      }
